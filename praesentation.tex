...
\institute{TU Wien -- Institut für Strömungsmechanik und Wärmeübertragung}
\date{2026}
...
$\Rightarrow$ Es existiert ein Geschwindigkeitspotential:
$\;\vec{v} = \nabla\varphi$

\smallskip
Einsetzen in die Kontinuitätsgleichung (inkompressibel):
\vspace{-1mm}
\[
  \nabla\cdot\vec{v} = 0
...
\vspace{-3mm}
Das ist die \textbf{Laplace-Gleichung} -- Grundlage der Panelmethode.
Die Kontur wird in $n$ gerade Panele unterteilt, auf denen
Singularitäten (Quellen, Wirbel) angesetzt werden.

\smallskip
\textbf{Untersuchte Profile} (UIUC-Datenbank, XFOIL 160\,Pkt.):
...
nicht physikalisch real).

\smallskip
Integration der Punktquell-Beiträge über jedes Panel ergibt
Einflusskoeffizienten, die beschreiben, wie stark Panel~$j$
die Geschwindigkeit am Mittelpunkt von Panel~$i$ beeinflusst.

\smallskip
Die $q_j$ folgen aus der \textbf{Undurchlässigkeitsbedingung}
($\vec{v}\!\cdot\!\vec{n}=0$ auf der Oberfläche) als lineares
Gleichungssystem.

\smallskip
Zunächst wird \emph{nur mit Quellen} gerechnet
(kein Auftrieb) $\to$ Validierung am \textbf{Kreiszylinder}.
\end{column}
\begin{column}{0.48\textwidth}
